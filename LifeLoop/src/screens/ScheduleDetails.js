...
const ScheduleDetails = () => {
  ...
  const fetchSchedule = async () => {
    try {
      ...
      const found = (res.data?.schedules || res.schedules || []).find(
        (s) => s._id === id,
      );
  ...
  const handleComplete = () => {
    Alert.alert("Mark Complete", "Mark this pickup as completed?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const response = await scheduleAPI.completeSchedule(id);
            const completedSchedule = response.data.schedule;

            Toast.show({ type: "success", text1: "Pickup completed! 🎉" });

            // Navigate to rating screen for the other party
            const otherUser =
              schedule.donor._id.toString() === user._id.toString()
                ? schedule.recipient
                : schedule.donor;
            ...
            setTimeout(() => {
              navigation.navigate("RateUser", {
                ...
                listingTitle: schedule.listing?.title || "Item",
                exchangeType:
                  schedule.donor._id.toString() === user._id.toString()
                    ? "donor"
                    : "recipient",
      ...
      >
        ...
        >
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={[s.statusBannerLabel, { color: meta.color }]}>
                {meta.label}
              </Text>
          ...
          {schedule.listing && (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={s.listingCardTitle} numberOfLines={1}>
          ...
          <View style={s.card}>
            ...
            <InfoRow
              icon="🕐"
              label="Time"
              value={schedule.proposedTime || schedule.confirmedTime}
            />
            <InfoRow
              icon="📍"
              label="Location"
              value={
                ...
                (typeof schedule.pickupLocation === "string"
                  ? schedule.pickupLocation
                  : schedule.pickupLocation?.address)
              }
            />
            <InfoRow
              icon="🎁"
              label="Donor"
              value={`${schedule.donor?.firstName || ""} ${schedule.donor?.lastName || ""}`.trim()}
            />
            <InfoRow
              icon="👤"
              label="Recipient"
              value={`${schedule.recipient?.firstName || ""} ${schedule.recipient?.lastName || ""}`.trim()}
              last
            />
          </View>
