...
const STATUS_COLORS = {
  ...
  confirmed: { color: "#4ade80", bg: "rgba(74,222,128,0.12)",  label: "CONFIRMED" },
...
const StatTile = ({ icon, value, label, color, delay }) => {
  ...
  useEffect(() => {
    Animated.spring(scaleAnim, { toValue: 1, delay, useNativeDriver: true, tension: 70, friction: 8 }).start();
  }, []);
  return (
...
const StatusChip = ({ status }) => {
  ...
  return (
    <View style={[s.statusChip, { backgroundColor: meta.bg, borderColor: meta.color }]}>
      <Text style={[s.statusChipText, { color: meta.color }]}>{meta.label}</Text>
    </View>
  );
};

const ScheduleCard = ({ schedule, userRole, onConfirm, onComplete, onCancel, onTrack, onPress }) => {
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim }}>
      <TouchableOpacity style={s.scheduleCard} onPress={onPress} activeOpacity={0.9}>
        {/* Card header */}
        <View style={s.scheduleCardHeader}>
          <View style={s.scheduleCardImageBox}>
            {schedule.listing?.images?.[0]
              ? <Image source={{ uri: schedule.listing.images[0] }} style={s.scheduleCardImage} />
          ...
          <View style={{ flex: 1, marginRight: 8 }}>
            ...
            <Text style={s.scheduleCardDateTime}>{formatDateTime(schedule)}</Text>
          </View>
        ...
        <View style={s.scheduleCardDetails}>
          <View style={s.scheduleCardDetailRow}>
            ...
            <Text style={s.scheduleCardDetailText} numberOfLines={1}>
              {schedule.pickupAddress || schedule.pickupLocation || "Location TBD"}
            </Text>
          </View>
          <View style={s.scheduleCardDetailRow}>
        ...
        {(canConfirm || canComplete || canCancel || canTrack) && (
          <View style={s.scheduleCardActions}>
            ...
            {canConfirm && (
              <TouchableOpacity style={s.actionBtnConfirm} onPress={onConfirm}>
                <Text style={s.actionBtnText}>✓ Confirm</Text>
              </TouchableOpacity>
            )}
            {canComplete && (
              <TouchableOpacity style={s.actionBtnComplete} onPress={onComplete}>
...
const Schedules = () => {
  ...
  useEffect(() => { if (!loading) fetchSchedules(); }, [activeTab, statusFilter]);
      ...
      { text: "Cancel", style: "cancel" },
      {
        text: "Confirm", onPress: async () => {
          try {
            ...
            fetchSchedules();
          } catch { Toast.show({ type: "error", text1: "Failed to confirm" }); }
        },
      },
    ]);
  };
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortBy) {
      case "date-asc":  return arr.sort((a, b) => new Date(a.proposedDate) - new Date(b.proposedDate));
      case "date-desc": return arr.sort((a, b) => new Date(b.proposedDate) - new Date(a.proposedDate));
      case "status":    return arr.sort((a, b) => ORDER[a.status] - ORDER[b.status]);
      default:          return arr;
    }
  }, [schedules, sortBy]);
  ...
  return (
    <SafeAreaView style={s.container}>
      <FlatList
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchSchedules(); }} tintColor="#4ade80" />}
        ListHeaderComponent={() => (
          <Animated.View style={{ opacity: headerAnim }}>
            ...
            {showSort && (
              <View style={s.sortDropdown}>
                {SORT_OPTIONS.map(opt => (
                  ...
                  >
                    <Text style={[s.sortOptionText, sortBy === opt.id && s.sortOptionTextActive]}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.statsRow}>
              <StatTile icon="📋" value={stats.total}     label="Total"     color="#94a3b8" delay={0}   />
              <StatTile icon="⏳" value={stats.pending}   label="Pending"   color="#fbbf24" delay={60}  />
              <StatTile icon="✅" value={stats.confirmed} label="Confirmed" color="#4ade80" delay={120} />
              <StatTile icon="🎉" value={stats.completed} label="Done"      color="#60a5fa" delay={180} />
            </ScrollView>
            ...
            <View style={s.tabRow}>
              {TABS.map(tab => (
                ...
                >
                  ...
                  <View style={[s.tabCount, activeTab === tab.id && s.tabCountActive]}>
                    <Text style={[s.tabCountText, activeTab === tab.id && s.tabCountTextActive]}>
                      {tabCounts[tab.id]}
                    </Text>
                  </View>
                </TouchableOpacity>
        ...
        renderItem={({ item }) => (
          <View style={s.cardWrap}>
            <ScheduleCard
              ...
              onTrack={() => navigation.navigate("Tracking", { scheduleId: item._id })}
              onPress={() => navigation.navigate("ScheduleDetails", { id: item._id })}
            />
          </View>
        )}
...
const s = StyleSheet.create({
  ...
  statTile: {
    width: 100, backgroundColor: "#131c2e", borderRadius: 16,
    padding: 14, marginRight: 10, borderWidth: 1, borderColor: "#1e2d45",
  ...
  tab: {
    ...
    backgroundColor: "#131c2e", borderRadius: 20,
    borderWidth: 1, borderColor: "#1e2d45",
  },
  ...
  tabCount: {
    backgroundColor: "#1e2d45", borderRadius: 10,
  ...
  filterChip: {
    backgroundColor: "#131c2e", borderRadius: 20,
    paddingHorizontal: 14, paddingVertical: 8,
    marginRight: 8, borderWidth: 1, borderColor: "#1e2d45",
  },
  filterChipActive:     { backgroundColor: "rgba(74,222,128,0.12)", borderColor: "#4ade80" },
  ...
  scheduleCardHeader: { flexDirection: "row", alignItems: "flex-start", marginBottom: 12, gap: 10 },
  ...
  scheduleCardTitle:     { fontSize: 14, fontWeight: "700", color: "#f1f5f9", marginBottom: 3 },
  ...
  scheduleCardActions: { flexDirection: "row", gap: 8, borderTopWidth: 1, borderTopColor: "#1e2d45", paddingTop: 12 },
  actionBtnTrack:   { flex: 1, paddingVertical: 9, borderRadius: 10, backgroundColor: "#3b82f6", alignItems: "center" },
  actionBtnConfirm: { flex: 1, paddingVertical: 9, borderRadius: 10, backgroundColor: "#4ade80", alignItems: "center" },
  actionBtnComplete:{ flex: 1, paddingVertical: 9, borderRadius: 10, backgroundColor: "#4ade80", alignItems: "center" },
  actionBtnCancel:  { width: 38, paddingVertical: 9, borderRadius: 10, backgroundColor: "#1e2d45", alignItems: "center" },
  actionBtnText:    { color: "#0a0f1e", fontWeight: "700", fontSize: 13 },
  actionBtnCancelText:{ color: "#f87171", fontWeight: "700", fontSize: 13 },

  // Status chip
  statusChip: { borderRadius: 8, paddingHorizontal: 9, paddingVertical: 4, borderWidth: 1 },
